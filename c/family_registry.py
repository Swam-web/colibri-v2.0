...
# (#P7) TTL cache for expensive host probes (nvidia-smi, rocm-smi, ldd...).
# `plan`, `doctor` and `tune` respawn them on every invocation; hardware and
# linkage do not change second to second, so memoize per process.
_PROBE_CACHE: dict = {}


def cached_probe(key, ttl_seconds, thunk):
    """Return thunk() cached for ttl_seconds (key must be hashable)."""
    now = time.monotonic()
