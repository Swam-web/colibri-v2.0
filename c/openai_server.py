...
class APIHandler(BaseHTTPRequestHandler):
    ...
    # (#P4) RAM cache for web/dist: key -> (mtime_ns, size, data, ctype, etag).
    # dist/ is written once at build time; stat() validates freshness.
    _STATIC_CACHE: dict = {}
    ...
    def serve_static(self, path):
        ...
        if entry is None or entry[0] != st.st_mtime_ns or entry[1] != st.st_size:
            # (#P4) read once, serve from RAM afterwards (dist/ changes only on rebuild)
            try:
            ...
            if old is not None:
                self._STATIC_CACHE_BYTES -= len(old[2])
            if self._STATIC_CACHE_BYTES + len(data) <= self._STATIC_CACHE_MAX_BYTES:
                self._STATIC_CACHE[key] = (st.st_mtime_ns, st.st_size, data, ctype, etag)
                self._STATIC_CACHE_BYTES += len(data)
            else:
                self._STATIC_CACHE[key] = (st.st_mtime_ns, st.st_size, data, ctype, etag)
        else:
            _, _, data, ctype, etag = entry
        ...
        self.send_header("Content-Length", str(len(data)))
        self.send_header("ETag", etag)
        # (#P4) hashed asset names are content-addressed -> immutable; entry page stays fresh
        if "/assets/" in key:
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == "/health":
                ...
                return
            if path == "/experts":
                # (#P8) ?since=N -> 304 sans corps quand rien n'a bougé : le polling
                # 1.5s ne re-télécharge plus ~100 Ko de hex à chaque tour.
                ...
                try:
                    since = int(dict(parse_qsl(urlsplit(self.path).query)).get("since", -1))
                except ValueError:
                    since = -1
                if since >= 0 and payload["seq"] <= since:
                    self.send_response(304)
                    self.send_cors_headers()
                    self.end_headers()
                    return
    ...
    def generation(self, body, prompt, request_id, chat, tools=None, tool_choice=None,
        ...
        with self.server.scheduler.admit(self.client_disconnected, cache_slot) as admission:
            ...
            # (#P6) SSE coalescing: buffer encoded events, flush at ~16KB or
            # every ~20ms instead of 1 write+flush syscall per token.
            _ev_buf = bytearray()
            _ev_last_flush = [time.time()]
            _EV_MAX_BYTES = 16384
            _EV_MAX_AGE = 0.02

            def flush_events():
                nonlocal connected
                if not connected or not _ev_buf:
                    return
                with ka_lock:
                    try:
                        self.wfile.write(bytes(_ev_buf))
                        self.wfile.flush()
                        last_write[0] = time.time()
                        _ev_last_flush[0] = last_write[0]
                    except OSError:
                        connected = False
                    finally:
                        del _ev_buf[:]
            ...
            def event(choices, usage_marker=False):
                ...
                _ev_buf += f"data: {data}\n\n".encode()
                if len(_ev_buf) >= _EV_MAX_BYTES or time.time() - _ev_last_flush[0] >= _EV_MAX_AGE:
                    flush_events()
            ...
            flush_events()   # (#P6) push remaining buffered deltas before [DONE]
            if connected:
    ...
    def anthropic_generation(self, body, prompt, request_id, tools, enable_thinking):
        ...
        with self.server.scheduler.admit(self.client_disconnected, cache_slot) as admission:
            ...
            last_write = [time.time()]
            ka_stop = threading.Event()
            # (#P6) same SSE coalescing as the OpenAI path above
            _ev_buf2 = bytearray()
            _ev_last_flush2 = [time.time()]

            def flush_send_events():
                if not connected[0] or not _ev_buf2:
                    return
                with write_lock:
                    try:
                        self.wfile.write(bytes(_ev_buf2))
                        self.wfile.flush()
                        last_write[0] = time.time()
                        _ev_last_flush2[0] = last_write[0]
                    except OSError:
                        connected[0] = False
                    finally:
                        del _ev_buf2[:]
            ...
            def send_event(name, payload):
                ...
                _ev_buf2 += f"event: {name}\ndata: {data}\n\n".encode()
                if len(_ev_buf2) >= 16384 or time.time() - _ev_last_flush2[0] >= 0.02:
                    flush_send_events()
            ...
            flush_send_events()   # (#P6) push remaining buffered deltas before close
            # close_connection was already set when the 200 was committed (#597 item 3).
