...
def _discover_nvidia_gpus():
    ...
    try:
        # (#P7) GPU inventory is stable for the life of the process: probe once
        result = cached_probe(("nvidia-smi", tuple(command)), 30, lambda: subprocess.run(
            command, text=True, capture_output=True, check=True, timeout=5))
...
def _discover_amd_gpus():
    ...
    try:
        # (#P7) see nvidia path above: probe once per process
        result = cached_probe(("rocm-smi", tuple(command)), 30, lambda: subprocess.run(
            command, text=True, capture_output=True, check=True, timeout=5))
    except (OSError, subprocess.SubprocessError):
        return []
    import csv
