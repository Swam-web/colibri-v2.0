{
  ...
  outputs = {
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      in {
        # Launcher GTK + bundle (moteur optimisé + launcher, coli dans le PATH).
        # Fichiers : launcher/app.py, launcher/icons/, launcher/nix/bundle.nix.
        packages = let
          launcher = import ./launcher/nix/bundle.nix { inherit pkgs colibri; };
        in {
          default = colibri;
          inherit colibri;
          colibri-launcher = launcher.app;
          colibri-bundle = launcher.bundle;
        };
        ...
        apps = {
          ...
          launcher = {
            type = "app";
            program = "${self.packages.${pkgs.stdenv.hostPlatform.system}.colibri-launcher}/bin/colibri-launcher";
          };
    ...
    // {
      # Module NixOS : bundle moteur optimisé + launcher + raccourci GNOME.
      # Usage : inputs.colibri.url = "github:Swam-web/colibri";
      #         modules = [ colibri.nixosModules.bundle ];
      nixosModules = {
        bundle = { config, lib, pkgs, ... }: {
          environment.systemPackages = [
            self.packages.${pkgs.stdenv.hostPlatform.system}.colibri-bundle
          ];
        };
        default = self.nixosModules.bundle;
      };
    };
}
