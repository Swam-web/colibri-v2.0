...
export async function getProfile(baseUrl: string, apiKey = "", signal?: AbortSignal, since = -1): Promise<ProfileResponse | null> {
  const url = serverEndpoint(baseUrl, "profile") + (since >= 0 ? `?since=${since}` : "")
  const response = await fetch(url, { headers: headers(apiKey), signal })
  if (response.status === 304) return null   // (#P8) inchangé: le caller garde son snapshot
  if (!response.ok) throw new Error(await responseError(response))
