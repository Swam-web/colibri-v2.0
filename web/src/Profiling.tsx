...
export function Profiling({ baseUrl, apiKey, connected }: { baseUrl: string; apiKey: string; connected: boolean }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        // (#P8) ?since= : 304 si rien de neuf, on garde le snapshot (0 parse, 0 re-render)
        const result = await getProfile(baseUrl, apiKey, undefined, lastProfileSeq.current)
        if (!disposed && result) {
          lastProfileSeq.current = result.seq
