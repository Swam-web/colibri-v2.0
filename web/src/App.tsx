...
// (#P5) code-split: Brain/Profiling (+ canvas 19k) hors du chunk initial du chat
const Brain = lazy(() => import("./Brain").then((m) => ({ default: m.Brain })))
const Profiling = lazy(() => import("./Profiling").then((m) => ({ default: m.Profiling })))
...
export default function App() {
  ...
  // (#P3) stream batching: deltas accumulate here, flushed to state ~12x/s
  // instead of 1 setState + full Markdown re-parse per token
  const streamBuf = useRef({ reasoning: "", content: "", count: 0, assistantId: "", t0: 0, firstToken: true })
  ...
  // EFFECT #7 — follow the stream only when the user is already at the bottom;
  // a smooth scroll per token hijacks reading and queues animations (jank)
  useEffect(() => {
    const el = bottomRef.current
    if (!el) return
    const scroller = el.parentElement
    if (!scroller) { el.scrollIntoView({ behavior: "auto" }); return }
    const nearBottom = scroller.scrollHeight - scroller.scrollTop - scroller.clientHeight < 160
    if (nearBottom) el.scrollIntoView({ behavior: "auto" })
  }, [messages])
  ...
  // (#P3) push the buffered deltas to state (single re-render per tick)
  const flushStream = () => {
    const b = streamBuf.current
    if (!b.assistantId || (!b.reasoning && !b.content)) return
    const { reasoning, content, count, assistantId, t0 } = b
    b.reasoning = ""
    b.content = ""
    ...
    updateMessages((current) => current.map((item) =>
      item.id === assistantId
        ? { ...item, reasoning: (item.reasoning ?? "") + reasoning, content: item.content + content }
        : item,
    ))
  }
  const scheduleFlush = () => {
    if (!flushTimer.current) flushTimer.current = window.setInterval(flushStream, 80)
  }
  const stopFlush = () => {
    if (flushTimer.current) { window.clearInterval(flushTimer.current); flushTimer.current = 0 }
    flushStream()
  }
  ...
  const send = async () => {
    ...
    streamBuf.current = { reasoning: "", content: "", count: 0, assistantId: assistant.id, t0, firstToken: true }
    const controller = new AbortController()
    ...
    try {
      const result = await streamChat({
        ...
        onReasoning: (delta) => {
          const b = streamBuf.current
          if (b.firstToken) { setTtft(performance.now() - t0); setStreamStart(performance.now()); b.firstToken = false }
          b.count++
          b.reasoning += delta
          scheduleFlush()
        },
      ...
      const finalCount = streamBuf.current.count
      if (finalCount > 0 && finalElapsed > 0) setTokPerSec(finalCount / finalElapsed)
      ...
      stopFlush()   // (#P3) push les derniers tokens bufferisés avant de fermer
  ...
  return (
    <div className="app-shell">
      ...
      <main className="chat-panel">
        ...
        {view === "brain" ? (
          <Suspense fallback={<div className="view-loading"><LoaderCircle className="size-5 animate-spin" /></div>}>
